...
public class DBVacancyDao implements VacancyDao {
  ...
  private static final String SQL_UPDATE_VACANCY =
      "UPDATE vacancy SET name=?, salary=?, currency=?, description=?, duties=?, conditions=?, employment_type=? "
          + "WHERE id_vacancy=?;";
  private static final String SQL_DELETE_VACANCY = "DELETE FROM vacancy WHERE id_vacancy=?;";
  private static final String SQL_ADD_TRANSLATION_VACANCY =
      "INSERT INTO tvacancy (id_vacancy, lang, name, description, duties, conditions) VALUES (?, ?, ?, ?, ?, ?);";
  private static final String SQL_UPDATE_TRANSLATION_VACANCY =
      "UPDATE tvacancy SET name=?, description=?, duties=?, conditions=? WHERE id_vacancy=? and lang=?;";
  private static final String SQL_SELECT_COUNT_VACANCY =
      "SELECT count(id_vacancy) as vacancy_count FROM vacancy;";
  private static final String SQL_SELECT_VACANCY_BY_ID =
      "SELECT * FROM vacancy WHERE id_vacancy=?;";
  private static final String SQL_SELECT_TRANSLATE_VACANCY_BY_ID =
      "SELECT v.id_vacancy, coalesce(tv.name, v.name) AS name, "
  ...
  private static final String SQL_SELECT_ONLY_TRANSLATE_VACANCY_BY_ID =
      "SELECT id_vacancy, name, description, duties, conditions FROM db_hr_system.tvacancy WHERE id_vacancy=?;";
  private static final String SQL_SELECT_ALL_TRANSL_VACANCY =
      "SELECT v.id_vacancy, coalesce(tv.name, v.name) AS name, "
          + "v.salary, v.currency, v. publish_date, coalesce(tv.description, v.description) AS description, coalesce(tv.duties, v.duties) AS duties, "
  ...
  private static final String SQL_SELECT_VACANCY_BY_HREMAIL =
      "SELECT * FROM vacancy WHERE email=? ORDER BY id_vacancy DESC LIMIT ?,?;";
  private static final String SQL_SELECT_TRANSL_VAC_BY_HREMAIL =
      "SELECT v.id_vacancy, coalesce(tv.name, v.name) AS name, "
          + "v.salary, v.currency, v. publish_date, coalesce(tv.description, v.description) AS description, coalesce(tv.duties, v.duties) AS duties, "
          + "coalesce(tv.conditions, v.conditions) AS conditions, v.employment_type, v.active, v.hot, v.email "
  ...
  private static final String SQL_SELECT_COUNT_VACANCY_BY_HR_EMAIL =
      "SELECT count(id_vacancy) as vacancy_count FROM vacancy WHERE email=?;";
  private static final String SQL_SELECT_VACANCY_LIKE =
      "SELECT * FROM vacancy WHERE active='active' and name LIKE ?;";
  // private static final String SQL_SELECT_TRASL_VACANCY_LIKE = "?";
  private static final String SQL_COUNT_ALL_ACTIVE_VACANCY =
      "SELECT count(id_vacancy) as countResume FROM vacancy WHERE active='active';";
  private static final String SQL_SELECT_ALL_ACTIVE_VACANCY =
      "SELECT * FROM vacancy WHERE active='active' ORDER BY publish_date DESC, id_vacancy LIMIT ?,?;";
  private static final String SQL_SELECT_ALL_TRANSL_ACTIVE_VACANCY =
      "SELECT v.id_vacancy, coalesce(tv.name, v.name) AS name, "
          + "v.salary, v.currency, v. publish_date, coalesce(tv.description, v.description) AS description, coalesce(tv.duties, v.duties) AS duties, "
          + "coalesce(tv.conditions, v.conditions) AS conditions, v.employment_type, v.active, v.hot, v.email "
  ...
  private static final String SQL_SELECT_ALL_HOT_VACANCY =
      "SELECT * FROM vacancy WHERE hot='hot' LIMIT 5;";
  private static final String SQL_SELECT_ALL_TRANSL_HOT_VACANCY =
      "SELECT v.id_vacancy, coalesce(tv.name, v.name) AS name, "
          + "v.salary, v.currency, v. publish_date, coalesce(tv.description, v.description) AS description, coalesce(tv.duties, v.duties) AS duties, "
          + "coalesce(tv.conditions, v.conditions) AS conditions, v.employment_type, v.active, v.hot, v.email "
          + "FROM vacancy AS v LEFT JOIN (SELECT * FROM tvacancy WHERE lang = ?) AS tv USING(id_vacancy)  WHERE hot='hot' LIMIT 7;";
  private static final String SQL_ACTIVATE_VACANCY =
      "UPDATE vacancy SET active='active', publish_date=? WHERE id_vacancy=?;";
  private static final String SQL_HOT_VACANCY = "UPDATE vacancy SET hot='hot' WHERE id_vacancy=?;";
  private static final String SQL_DEACTIVATE_VACANCY =
      "UPDATE vacancy SET active='non active', hot='non hot' WHERE id_vacancy=?;";
    ...
    try {
      ...
      if (rs.next()) {
        ...
        vacancy.setEmploymentType(
            EmploymentType.valueOf(rs.getString(9).toUpperCase().replace(' ', '_')));
  ...
  @Override
  public List<Vacancy> selectVacancyByHrEmail(
      String hrEmail, String lang, int pageNum, int amountPerPage) throws DAOException {
    List<Vacancy> vacancy = new ArrayList<Vacancy>();
    Connection conn = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
  ...
  public List<Vacancy> selectAllActiveVacancy(String lang, int pageNum, int amountPerPage)
    ...
    logger.debug(
        "DBVacancyDAO.selectAllActiveResume() - pageNum={},amountPerPage={},vacancy = {}",
        pageNum,
        amountPerPage,
        vacancyList);
    return vacancyList;
  ...
  private Vacancy getVacacnyFromResultSet(ResultSet set) throws SQLException {
    ...
    vacancy.setEmploymentType(
        EmploymentType.valueOf(set.getString(9).toUpperCase().replace(' ', '_')));
