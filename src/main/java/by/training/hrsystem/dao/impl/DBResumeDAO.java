...
public class DBResumeDao implements ResumeDao {
  ...
  private static final String SQL_UPDATE_RESUME =
      "UPDATE resume SET name=?, publish_date=?, military=? WHERE id_resume=?;";
  private static final String SQL_DELETE_RESUME = "DELETE FROM resume WHERE id_resume=?;";
  private static final String SQL_SELECT_COUNT_RESUME =
      "SELECT count(id_resume) AS resume_count FROM resume;";
  private static final String SQL_ACTIVATE_RESUME =
      "UPDATE resume SET active=active WHERE id_resume=?;";
  private static final String SQL_SELECT_RESUME_BY_ID = "SELECT * FROM resume WHERE id_resume=?;";
  private static final String SQL_SELECT_RESUME_BY_APPLICANT =
      "SELECT * FROM resume WHERE email=? LIMIT ?,?;";
  private static final String SQL_SELECT_RESUME_FOR_VACANCY = "SELECT * FROM resume WHERE email=?;";
  private static final String SQL_SELECT_COUNT_RESUME_BY_APPLIC_EMAIL =
      "SELECT count(id_resume) AS resume_count FROM resume WHERE email=?;";
  private static final String SQL_SELECT_RESUME_BY_ID_VACANCY =
      "SELECT r.id_resume, r.name, r.publish_date, r.military, r.active, r.email FROM verify as v, resume as r "
          + "WHERE v.id_vacancy=? and r.id_resume=v.id_resume; ";
  private static final String SQL_SELECT_LEFT_RESUME =
      "SELECT * FROM resume as r JOIN verify as v ON r.id_resume=v.id_resume WHERE id_vacancy=? and email=?;";
  ...
  public List<Resume> selectResumeByApplicant(String applicantEmail, int pageNum, int amountPerPage)
      throws DAOException {
    logger.debug(
        "DBResumeDAO.selectResumeByApplicant() - applicantEmail = {}, lang={}, pageNum={},amountPerPage",
        applicantEmail,
        pageNum,
        amountPerPage);
  ...
  public List<Resume> selectLeftResume(int idVacancy, String applicantEmail) throws DAOException {
    logger.debug(
        "DBResumeDAO.selectleftResume() - idVacancy={}, applicantEmail = {}",
        idVacancy,
        applicantEmail);
    List<Resume> resume = new ArrayList<Resume>();
    Connection conn = null;
