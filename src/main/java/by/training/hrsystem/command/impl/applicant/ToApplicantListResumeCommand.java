    ...
    if (user != null && user.getRole() == Role.APPLICANT) {
      ...
      try {
        List<Resume> resumeList =
            resumeService.selectResumeByEmail(
                applicantEmail.getEmail(), (pageNumber - 1) * RESUME_PER_PAGE, RESUME_PER_PAGE);
        ...
        request
            .getRequestDispatcher(PageName.APPLICANT_LIST_RESUME_PAGE)
